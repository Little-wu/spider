...
def main():
    ...
    for url in star_urls:
        ...
        js = get_user(url, 1, 0)
        # 创建用户文件夹
        if os.path.exists(os.path.join(os.getcwd(), 'user', js.get('post_list')[0].get('author_id'))):
            continue
        create_dir(os.path.join(os.getcwd(), 'user', js.get('post_list')[0].get('author_id')))
        while True:
            post_list = parse_user(js)
            for item in post_list:
                # 为每个图集创建文件夹
                create_dir(
                    os.path.join(os.getcwd(), 'user', js.get('post_list')[0].get('author_id'), item.get('post_id')))
                for image_url in get_image_urls(item):
                    time.sleep(0.5)
                    download_image(image_url, os.path.join(os.getcwd(), 'user', js.get('post_list')[0].get('author_id'),
                                                           item.get('post_id')))
            if js.get('more'):
                count = count + 1
